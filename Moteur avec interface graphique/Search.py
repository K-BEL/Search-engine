...
def main():
    ...
    base_dir = r"E:\Downloads\cours\S5\Technologie de moteurs de recherche\Projet\Moteur de recherche\Projet\projectCorpus"
    file_list = os.listdir(base_dir)
    
    dict = create_dict(base_dir)
    pdict = create_permuterm(dict.keys())
    
    print ("Appuyez sur 1 pour la recherche par requête ; 2 pour générer une table d'index global ; 3 pour générer la table d'index permuterm")
    choice = input('Votre choix:')
    
    if choice == '1':
    
        while 1:
            input_query = input("Entrer la requete ")
            
            result = []
            if input_query == "":
                print ("Entrer une requete non vide :")
            else:
